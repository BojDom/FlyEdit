...
var defaultExcludeDirs = ["**", '!**node_modules**', "!**.git**", "!dist"];
...
function toTmp(f) {
	var from = upath.join(FEconfig.server.root, f);
	var to = upath.join(tmpFolder, f);

	return new Promise((resolve, reject) => {
		if (tmpFolder) {
			sshConn.exec('mkdir -p ' + (upath.join(to, '../')), (err1, stdout1, stderr1) => {
...
function watchProject() {
	...
	watcher = watch.createMonitor(FEconfig.localRoot, {
		...
		filter: (name) => {
			return (multimatch([name], excludeDirs).length === 1)
		}
	}, function (monitor) {
		...
		monitor.on("created", function (f, stat) {
			...
			if (is('file', stat.mode)) {
				f = f.replace(FEconfig.localRoot, '');
				upload(f);
			}
		});
